# app.py — 沖ドキ！BLACK 押し引き補助 v0.51
# - 区切り=スライダー＋決定（チェックボックス廃止）
# - サイドバーは「変更を反映」で適用（未反映警告つき）
# - サマリーは省略記号が出ないようCSS調整
...
def load_model_table() -> Optional[pd.DataFrame]:
    ...
    def pct_range(lbl: str) -> Tuple[float, float]:
        ...
        if len(m) == 2: a, b = float(m[0]), float(m[1]); return (min(a,b), max(a,b))
...
def pick_model(short_pct: float, adv_g: float) -> Dict:
    """レンジ一致 → 最近傍 の順で1行返す。short_pct は 0-100[%]。"""
...
# ------------------------ Sidebar: 設定（反映ボタン） ------------------------
DEFAULT_CONF = {
    "base_50": 32, "coin_in_g": 3.00,
    "big_pay": 210, "reg_pay": 90,
    "big_adv_g": 59, "reg_adv_g": 24,
    "heaven_avg_diff": 100,      # 天国1回あたり平均差枚（EV用）
    "ev_risk_limit": 300,        # 攻めの許容マイナス
}
...
with c_del2:
    if st.button("1つ削除（末尾）", use_container_width=True):
        if st.session_state.hist_rows: push_undo(); st.session_state.hist_rows.pop(); st.rerun()

# エディタ結果 → state へ戻す（セグ維持）
df_after = pd.DataFrame(grid); m = len(df_after)
...
# 残り値クランプ
v = int(st.session_state.get("seg_slider_v051", 1))
st.session_state.seg_slider_v051 = max(1, min(v, max(1, N)))
if N > 0:
    seg_pos_1 = st.slider("区切り行（1〜N）", 1, N, value=st.session_state.seg_slider_v051,
...
def calc_metrics(rows: List[Dict], now_hamari_g: int, conf: Dict) -> Dict:
    ...
    reg_n    = sum(1 for r in rows if str(r.get("Type","BIG")) == "REG")
    ...
    adv_g = (total_iv
             + big_n * int(conf["big_adv_g"])
             + reg_n * int(conf["reg_adv_g"])
             + int(now_hamari_g))
    ...
    ev = p_trig * (e_len * float(conf["heaven_avg_diff"])) \
         - (int(now_hamari_g) * float(conf["coin_in_g"]))

    return {
        "coin_in":   int(round(coin_in)),
        "payout":    int(payout),
